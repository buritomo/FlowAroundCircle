...
#endif
    for (kx = 0; kx < II_STEP - 1; kx++) {
        for (ky = 0; ky < JJ_STEP - 1; ky++) {
            int k = kx + ky * II_STEP;
...
#ifdef PARA
#pragma omp parallel for private(lx, ly)
#endif
    for (int k = 0; k < 4; k++) {
        for (lx = 2; lx < II_STEP - 2; lx++) {
            for (ly = 0; ly < JJ_STEP - 1; ly++) {
                int l = lx + ly * II_STEP;
                for (int m = 0; m < 4; m++) {
...
#endif
    for (int k = 0; k < 4; k++) {
        for (lx = 0; lx < II_STEP - 1; lx++) {
            for (ly = 2; ly < JJ_STEP - 2; ly++) {
                int l = lx + ly * II_STEP;
                for (int m = 0; m < 4; m++) {
